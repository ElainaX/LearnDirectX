...
void Draw(const GameTimer& gt)
{
	...
	mCmdList->DrawIndexedInstanced(mBoxGeo->DrawArgs["box"].IndexCount, 1,
		mBoxGeo->DrawArgs["box"].StartIndexLocation, mBoxGeo->DrawArgs["box"].BaseVertexLocation, 0);
...
void BuildBoxGeometry()
{
	...
	mBoxGeo = std::make_shared<MeshGeometry>();
	mBoxGeo->Name = "boxGeo";
	...
	mBoxGeo->VertexBufferGPU = CreateDefaultBuffer(m_Device.Get(), mCmdList.Get(), vertices.data(), vbByteSize, mBoxGeo->VertexBufferUploader);
	mBoxGeo->IndexBufferGPU = CreateDefaultBuffer(m_Device.Get(), mCmdList.Get(), indices.data(), ibByteSize, mBoxGeo->IndexBufferUploader);
